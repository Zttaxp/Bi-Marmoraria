...
export default function AnnualReport({ data, isVisible }: { data: any[], isVisible: boolean }) {
  ...
  const fetchFinancials = useCallback(async () => {
      ...
      // CORREÇÃO CRUCIAL: Ordenamos por ID ascendente (Antigo -> Novo)
      // Assim, ao iterar, o registro mais novo sempre sobrescreve os antigos no mapa.
      ...
      if (dbData) {
          dbData.forEach((row: any) => { 
              map[row.month_key] = row // O último loop (registro mais novo) vence
          })
      }
