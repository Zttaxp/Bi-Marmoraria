...
import { Users, Briefcase, Crown, Search, X, Layers, AlertCircle, Filter, Target, Loader2 } from 'lucide-react'
import { createClient } from '../app/utils/supabase/client'

export default function SellerAnalysis({ data, showGoals }: { data: any[], showGoals: boolean }) {
  const supabase = createClient()
  
  // --- HOOKS (Sempre no topo, sem interrupções) ---
  ...
  // 1. BUSCAR METAS DO BANCO (useEffect movido para cima)
  useEffect(() => {
  ...
  // 2. EXTRAIR LISTA DE VENDEDORES (useMemo)
  ...
  // 4. PROCESSAMENTO DOS DADOS (useMemo)
  const { rankingSellers, rankingClientsTotal, rankingClientsHigh } = useMemo(() => {
      ...
      const rClientsTotal = Object.values(clientsMap)
        .map((c: any) => ({ ...c, margin: calcMargin(c.revenue, c.cost, c.freight) }))
        .sort((a: any, b: any) => b.revenue - a.revenue)
        .slice(0, 10)
      ...
      return { rankingSellers: rSellers, rankingClientsTotal: rClientsTotal, rankingClientsHigh: rClientsHigh }
  ...
  const getModalContent = () => {
      ...
      selectedItem.items.forEach((sale: any) => {
          if (modalType === 'CLIENT' && selectedItem.revenueHigh && !sale.isHigh && selectedItem.revenue === selectedItem.revenueHigh) return;
  ...
  // AGORA SIM: Se não tiver dados, retornamos null ou aviso, MAS DEPOIS DE TODOS OS HOOKS
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
         <div className="bg-white p-3 rounded-xl shadow-sm border border-slate-100 flex items-center gap-4 flex-1 w-full md:w-auto">
             <div className="flex items-center gap-2 text-slate-700">
                ...
                <span className="font-bold text-sm hidden md:inline">Vendedor:</span>
             </div>
             <select
                value={selectedSellerFilter}
                onChange={(e) => setSelectedSellerFilter(e.target.value)}
                className="flex-1 p-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-700 text-sm font-medium focus:ring-2 focus:ring-cyan-500 outline-none cursor-pointer"
            >
         ...
         <button 
            ...
            className="flex items-center gap-2 bg-slate-800 text-white px-4 py-3 rounded-xl font-bold text-sm hover:bg-slate-700 transition shadow-sm w-full md:w-auto justify-center"
         >
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="p-4 bg-slate-50 border-b border-slate-100 flex items-center gap-2">
            ...
            <h3 className="font-bold text-slate-700">Performance & Metas</h3>
        </div>
            ...
            <table className="w-full text-sm text-left">
                <thead className="bg-white text-slate-500 font-bold border-b border-slate-100">
                    <tr>
                        ...
                        {/* SÓ MOSTRA SE SHOWGOALS FOR TRUE (MODO MÊS) */}
                ...
                <tbody className="divide-y divide-slate-50">
                    {rankingSellers.map((seller: any, idx: number) => (
                        <tr 
                            key={idx} 
                            onClick={() => { setSelectedItem(seller); setModalType('SELLER'); }}
                        ...
                        >
                                    ...
                                    {seller.goal > 0 ? (
                                        <div className="flex flex-col gap-1">
                                            <div className="flex justify-between text-xs font-bold text-slate-600">
                                                ...
                                                <span className="text-slate-400">Meta: {seller.goal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', maximumFractionDigits: 0 })}</span>
                                            </div>
                                            <div className="w-full bg-slate-200 rounded-full h-2 overflow-hidden">
                                                <div 
                                                    className={`h-full rounded-full transition-all duration-1000 ${
                                                        seller.progress >= 100 ? 'bg-green-500' : 
                                                        seller.progress >= 70 ? 'bg-cyan-500' : 'bg-orange-400'
                                                    }`}
                            ...
                            <td className="p-4 text-right">
                                <BadgeMargin value={seller.margin} />
                            </td>
                        </tr>
                    ))}
                    {rankingSellers.length === 0 && (
                        <tr><td colSpan={showGoals ? 7 : 6} className="p-8 text-center text-slate-400">Nenhum dado encontrado para este filtro.</td></tr>
                    )}
                </tbody>
            </table>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden flex flex-col">
            <div className="p-4 bg-purple-50 border-b border-purple-100">
                <h3 className="font-bold text-purple-800 flex items-center gap-2">
                    <Crown size={18} /> 
                    {selectedSellerFilter ? 'Clientes (Alto Valor)' : 'Top Clientes (Alto Valor)'}
                </h3>
            </div>
            ...
            <div className="bg-white w-full max-w-md rounded-xl shadow-2xl flex flex-col animate-in zoom-in-95 max-h-[80vh]">
                <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50 rounded-t-xl">
                    <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                        <Target size={20}/> Definir Metas Mensais
                    </h2>
                ...
                <div className="p-6 overflow-y-auto flex-1 space-y-4">
                    <p className="text-xs text-slate-500 mb-4 bg-yellow-50 p-2 rounded border border-yellow-100">
                        Insira a <b>Meta Mensal Padrão</b>. Ela será mostrada e comparada apenas quando o filtro for "Por Mês".
                    </p>
                    {sellersList.map((seller) => (
                        <div key={seller} className="flex flex-col gap-1">
                            ...
                            <div className="relative">
                                ...
                                <input 
                                    type="number"
                                    defaultValue={goals[seller] || ''}
                                    onBlur={(e) => handleSaveGoal(seller, e.target.value)}
                                    placeholder="0,00"
                                    className="w-full pl-9 pr-3 py-2 border border-slate-200 rounded focus:ring-2 focus:ring-cyan-500 outline-none"
                                />
                            </div>
                        </div>
                    ))}
                ...
                <div className="p-4 border-t border-slate-100 bg-slate-50 rounded-b-xl text-right">
                    <button onClick={() => setIsGoalsModalOpen(false)} className="px-4 py-2 bg-cyan-600 text-white font-bold rounded hover:bg-cyan-700">Concluir</button>
                </div>
            </div>
        </div>
      )}
                ...
                <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50 rounded-t-xl">
                    <div>
                        ...
                        <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                            {modalType === 'SELLER' ? <Briefcase size={20}/> : <Users size={20}/>} {selectedItem.name}
                        </h2>
                    </div>
...
function ClientTable({ items, color, onClick }: { items: any[], color: string, onClick: (item: any) => void }) {
    const textColors: any = { blue: "text-blue-700", purple: "text-purple-700" }
    return (
        <div className="overflow-x-auto flex-1">
            <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-slate-50">
                    ...
                    {items.length === 0 && <tr><td colSpan={4} className="p-6 text-center text-slate-400">Sem dados neste período.</td></tr>}
                </tbody>
            </table>
        </div>
    )
